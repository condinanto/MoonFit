...
class AdminError(BotError):
    """Error for admin operation failures"""
    pass

def handle_bot_error(error: Exception) -> str:
    """Convert exceptions to user-friendly error messages"""
    if isinstance(error, ValidationError):
        return f"❌ Invalid input: {str(error)}"
    elif isinstance(error, PaymentError):
        return f"💳 Payment error: {str(error)}"
    elif isinstance(error, AdminError):
        return f"🔧 Admin error: {str(error)}"
    else:
